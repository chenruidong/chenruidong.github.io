# Site Copy Review

Edit this file directly. Keep the section labels so I can map your revisions back into the static site quickly.

## Site Meta

Site title:
Ruidong Chen

Meta description:
Ruidong Chen studies how neural circuits generate flexible behavior from minimal experience and observation.

## Header

Wordmark:
None (removed)

Navigation:
- Cognitive Flexibility
- News
- Research
- CV

News route:
#news (in-site tab view)

CV route:
#cv (in-site view)

CV PDF source:
downloads/Chen-CV-2026.pdf

Language toggle:
- zh button: 中
- en button: EN

Header social icons (desktop order):
- Email
- GitHub
- X
- Bluesky
- LinkedIn
- Zhihu
- Xiaoyuzhou
- Apple Podcasts
- RSS

Placement:
In the single header row, to the right of the text navigation, with language toggle at the far right.

Small-screen behavior:
Hide icons only when nav space is actually tight; remove from the right one-by-one (RSS first) to preserve a single-row nav. On very narrow screens, Email and X are kept longest.

## CV View

Section kicker:
CV

Section intro:
(RC: delete this no intro needed)

Loading text:
Loading CV preview...

Primary action label:
Open in new tab

Secondary action label:
Download PDF

## Home View

### Hero

Section id:
cognitive-flexibility

Eyebrow:
Neuroscience of flexible cognition

Intro paragraph:
Primates like humans and monkeys adapt to new situations quickly. The secret is learning generalizable rules from a small number of samples. A monkey can learn to rinse gritty sweet potato by observing another individual. A person can learn to play a new game in a few tries, unlike AI programs. How does the brain implement this cognitive flexibility? We still lack a mechanistic account. My long-term vision is to explain how neural circuits generate flexible behavior by observing, manipulating, and recreating computational elements in the primate brain.

### News Preview

Section id:
news-preview

Section kicker:
News

Section intro:
(RC: remove this)

Item 1 date:
2026.07.18

Item 1 announcement:
I will co-chair the Neurobiology of Cognition GRS. Please apply!

Item 1 link label:
Conference page

Item 1 link URL:
https://www.grc.org/neurobiology-of-cognition-grs-conference/2026

Item 2 date:
2026.02.28

Item 2 announcement:
I was interviewed by ScienceNet.cn.

Item 2 link label:
Read the interview

Item 2 link URL:
https://news.sciencenet.cn/htmlnews/2026/2/560639.shtm

Item 3 date label:
...

Item 3 link label:
Read more

Item 3 link URL:
#news

### Research Preview

Section kicker:
Research

Section intro:
(delete the intro)

Area 1 label:
Experience and observation

Area 1 paper meta:
Nature / 2026

Area 1 paper title:
Evidence accumulation from experience and observation in the cingulate cortex

Area 1 paper PDF:
assets/papers/chen_nature_2026.pdf

Area 2 label:
Song learning and motivation

Area 2 paper 1 meta:
Current Opinion in Neurobiology / 2020

Area 2 paper 1 title:
Actor-critic reinforcement learning in the songbird

Area 2 paper 1 PDF:
assets/papers/chen_current_opinion_in_neurobiology_2020.pdf

Area 2 paper 2 meta:
Neuron / 2019

Area 2 paper 2 title:
Songbird Ventral Pallidum Sends Diverse Performance Error Signals to Dopaminergic Midbrain

Area 2 paper 2 PDF:
assets/papers/chen_neuron_2019.pdf

Area 2 paper 3 meta:
Science / 2016

Area 2 paper 3 title:
Dopamine neurons encode performance error in singing birds

Area 2 paper 3 PDF:
assets/papers/gadagkar_science_2016.pdf

Area 3 label:
Singing and movement modules

Area 3 paper 1 meta:
Journal of Neurophysiology / 2021

Area 3 paper 1 title:
Movement signaling in ventral pallidum and dopaminergic midbrain is gated by behavioral state in singing birds

Area 3 paper 1 PDF:
assets/papers/chen_journal_of_neurophysiology_2021.pdf

Area 3 paper 2 meta:
Scientific Reports / 2018

Area 3 paper 2 title:
Place preference and vocal learning rely on distinct reinforcers in songbirds

Area 3 paper 2 PDF:
assets/papers/murdoch_scientific_reports_2018.pdf

## News View

Section id:
news

Section kicker:
News

Item 1 date:
2026.07.18

Item 1 announcement:
I will co-chair the Neurobiology of Cognition GRS. Please apply!

Item 1 link label:
Conference page

Item 1 link URL:
https://www.grc.org/neurobiology-of-cognition-grs-conference/2026

Item 2 date:
2026.02.28

Item 2 announcement:
I was interviewed by ScienceNet.cn.

Item 2 link label:
Read the interview

Item 2 link URL:
https://news.sciencenet.cn/htmlnews/2026/2/560639.shtm

Item 3 date:
2026.01.31

Item 3 announcement:
Online seminar hosted by iBrain.

Item 3 first link label:
Watch on YouTube

Item 3 first link URL:
https://www.youtube.com/watch?v=y4RyTX0tq8s

Item 3 second link label:
Watch on Bilibili

Item 3 second link URL:
https://www.bilibili.com/video/BV1ftFhzeEVP/?spm_id_from=333.1387.collection.video_card.click

Item 4 date:
2026.01.07

Item 4 announcement:
My first postdoc paper is published in Nature.

Item 4 first link label:
Read the paper

Item 4 first link URL:
https://www.nature.com/articles/s41586-025-09885-0

Item 4 second link label:
Read more

Item 4 second link URL:
#research-observation

Item 5 date:
2025.05.04

Item 5 announcement:
I started a new podcast with Sophon: Two of Coins.

Item 5 English link label:
Listen on Apple Podcasts

Item 5 English link URL:
https://podcasts.apple.com/us/podcast/%E4%B8%A4%E5%9D%97%E9%92%B1/id1812355099

Item 5 Chinese link label:
在小宇宙收听

Item 5 Chinese link URL:
https://www.xiaoyuzhoufm.com/podcast/6817c030601614cb6bbd0d6a

## Research View

Section kicker:
Research

### Research Area 1

Tab label:
Experience and observation

Title:
Evidence integration between self-experience and observation in the cingulate cortex

Paragraph 1:
We asked how observational learning interacts with evidence accumulation in the primate brain. In a two-player task, subjects alternated between acting and observing while both accumulated evidence. Behavior in humans and monkeys revealed a reliable asymmetry: observation is less effective than direct experience even when visual information and motivational state is matched.

Paragraph 2:
Electrophysiology in anterior cingulate cortex showed that experienced and observed evidence are both encoded and projected onto a shared belief axis, with a biased alignment toward self-experience. Population geometry and recurrent neural network modeling suggest this bias emerges from computations within ACC dynamics, providing a mechanistic account of why learning from others is systematically discounted.

Paper meta:
Nature / 2026

Paper title:
Evidence accumulation from experience and observation in the cingulate cortex

Paper PDF:
assets/papers/chen_nature_2026.pdf

Support link label:
News article

Support link URL:
https://mcgovern.mit.edu/2026/01/07/learning-from-each-other/

Support link label:
Chinese summary

Support link URL:
https://zhuanlan.zhihu.com/p/1992203318399898641

### Research Area 2

Tab label:
Song learning and motivation

Title:
Reinforcement learning by intrinsic motivation

Paragraph 1:
Reinforcement learning is one of the most successful algorithmic frameworks discovered in neuroscience. Animals refine action policies through trial and error to maximize reward. But what counts as reward? Is it limited to direct rewards like food and water? In a series of behavioral and neurophysiological experiments in zebra finches, we examined how dopaminergic prediction-error signals support song learning. We found that dopamine neurons encode performance prediction error during singing, and that ventral pallidum is a critical source of these signals.

Paragraph 2:
These findings motivated an actor-critic framework for song learning that links multiple brain regions into a circuit-level account of how intrinsic performance errors drive learning.

Paper meta:
Current Opinion in Neurobiology / 2020

Paper title:
Actor-critic reinforcement learning in the songbird

Paper PDF:
assets/papers/chen_current_opinion_in_neurobiology_2020.pdf

Paper meta:
Neuron / 2019

Paper title:
Songbird Ventral Pallidum Sends Diverse Performance Error Signals to Dopaminergic Midbrain

Paper PDF:
assets/papers/chen_neuron_2019.pdf

Paper meta:
Science / 2016

Paper title:
Dopamine neurons encode performance error in singing birds

Paper PDF:
assets/papers/gadagkar_science_2016.pdf

### Research Area 3

Tab label:
Multiple motivational drives

Title:
Organization of behavior modules with distinct motivational drives

Paragraph 1:
Biological systems orchestrate multiple goals, such as performing a motor skill and staying close to the flock. How are specialized behavioral modules organized? In zebra finches, singing and locomotion can be studied together, making it possible to compare the motivational drive of distinct behaviors. Our experiments showed that the same broadband noise stimulus can negatively reinforce vocal variants while positively reinforce locomotion, showing multiple intrinsic motivations at work, appropriately routed to distinct behavioral output modules.

Paragraph 2:
Neural analyses from ventral pallidum and dopaminergic midbrain further showed that timing signals can switch between song and movement on sub-second timescales. This supports a view of basal ganglia and midbrain circuits as flexibly gated across behavioral contexts rather than fixed to one function.

Paper meta:
Journal of Neurophysiology / 2021

Paper title:
Movement signaling in ventral pallidum and dopaminergic midbrain is gated by behavioral state in singing birds

Paper PDF:
assets/papers/chen_journal_of_neurophysiology_2021.pdf

Paper meta:
Scientific Reports / 2018

Paper title:
Place preference and vocal learning rely on distinct reinforcers in songbirds

Paper PDF:
assets/papers/murdoch_scientific_reports_2018.pdf

## Footer

(remove the footer)
