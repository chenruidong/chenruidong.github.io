# 中文文案校对

请直接编辑这个文件。保留各节标题，便于我把修订内容映射回静态页面。

## 站点元信息

站点标题：
陈睿东

元描述：
陈睿东研究神经回路如何从经验与观察中学习，并形成灵活的内部模型。

## 页眉

站点标识：
陈睿东

导航：
- 研究
- 链接

语言切换：
- 中文按钮：中
- 英文按钮：EN

## 首屏

肩题：
系统与计算神经科学

主标题：
简介

简介段落：
陈睿东是一位神经科学家，其研究跨越鸣禽、鹦鹉和灵长类动物，连接神经回路机制、行为与理论。他关注大脑如何生成预测、如何将行为组织为灵活的模块，以及如何将亲身经验与观察他人所获得的证据整合起来。

主按钮：
查看代表作

次按钮：
邮箱

首屏摘要条目：
- MIT Jazayeri 实验室博士后研究员
- 康奈尔大学神经生物学与行为学博士
- 香港大学计算机科学工学学士

## 主要研究

分节肩题：
主要研究

分节标题：
代表性工作。

### 项目 1

标题：
扣带皮层中自我经验与观察学习的证据整合

段落 1：
我们研究观察学习如何与灵长类大脑中的证据积累相互作用。在一个双人任务中，受试者轮流执行和观察，同时双方都在积累证据。人类和猴子的行为数据显示出稳定的不对称性：即使视觉信息和动机状态匹配，观察学习的效率仍低于直接经验。

段落 2：
前扣带皮层的电生理记录显示，亲身经验和观察所得的证据都会被编码，并投射到共享的信念轴上，但其对齐方式偏向自我经验。群体几何分析和循环神经网络建模进一步表明，这种偏差来源于 ACC 内部动力学中的计算机制，为我们为何会系统性低估从他人处学习提供了机制解释。

代表作标签：
代表作

论文：
扣带皮层中来自经验与观察的证据积累

作者：
Chen R*, Radkani S*, Valluru N, Yoo SBM, Jazayeri M.

期刊与年份：
Nature, 2026

简介：
展示了 ACC 如何将亲历与观察到的证据整合为共享的信念表征，并对自我经验赋予更高权重。

文章按钮：
文章

文章链接：
https://doi.org/10.1038/s41586-025-09885-0

### 项目 2

标题：
内在动机驱动的强化学习

段落 1：
我在鸣禽系统中的工作考察了多巴胺预测误差如何支持发声学习。基于此前关于投射至 Area X 的 VTA 神经元在鸣唱过程中编码表现预测误差的证据，这一研究进一步确定腹侧苍白球是这些信号的关键来源。通过损毁、逆行病毒追踪、微刺激以及体内单单位记录，我证明了腹侧苍白球对于歌曲学习是必需的；它接收来自歌曲时序与误差相关区域的输入，并向多巴胺中脑发送多样化的表现误差信号。

段落 2：
这些发现促成了一个关于歌曲学习的 actor-critic 框架，将多个脑区连接到同一回路层面的解释中，以说明内在表现误差如何驱动学习。

代表作标签：
代表作

论文：
鸣禽中的 actor-critic 强化学习

作者：
Chen R, Goldberg J.

期刊与年份：
Current Opinion in Neurobiology, 2020

简介：
提出了一个关于歌曲学习的回路层面 actor-critic 解释框架。

文章按钮：
文章

文章链接：
https://linkinghub.elsevier.com/retrieve/pii/S0959438820301173

论文：
鸣禽腹侧苍白球向多巴胺中脑发送多样化的表现误差信号

作者：
Chen R, Puzerey P, Roeser A, Riccelli T, Podury A, Maher K, Farhang A, Goldberg J.

期刊与年份：
Neuron, 2019

简介：
鉴定腹侧苍白球是向多巴胺中脑发送表现误差信号的重要来源。

文章按钮：
文章

文章链接：
https://linkinghub.elsevier.com/retrieve/pii/S0896627319303952

论文：
鸣唱鸟中的多巴胺神经元编码表现误差

作者：
Gadagkar V, Puzerey P, Chen R, Baird-Daniel E, Farhang A, Goldberg J.

期刊与年份：
Science, 2016

简介：
证明了投射至基底神经节的多巴胺神经元在鸣唱过程中编码表现误差。

文章按钮：
文章

文章链接：
https://www.science.org/doi/10.1126/science.aah6837

### 项目 3

标题：
鸟类脑与行为中鸣唱与运动模块的组织

段落 1：
我在博士阶段的第二个项目关注专门化行为模块如何被组织与协调。在斑胸草雀中，鸣唱和运动可以被同时研究，因此能够比较不同表现之间的强化与时序。行为实验表明，同一种宽带噪声刺激既可以负向强化发声变体，也可以在位置偏好任务中正向强化运动行为，这说明存在多种内在动机。

段落 2：
对腹侧苍白球和多巴胺中脑的神经活动分析进一步表明，时序信号可以在亚秒时间尺度上在鸣唱与运动之间切换。由此得到的图景是：基底神经节和中脑回路并非固定服务于单一功能，而是能够在不同的行为情境中被灵活门控。

代表作标签：
代表作

论文：
鸣唱鸟中腹侧苍白球和多巴胺中脑的运动信号受行为状态门控

作者：
Chen R, Gadagkar V, Roeser A, Puzerey P, Goldberg J.

期刊与年份：
Journal of Neurophysiology, 2021

简介：
证明神经时序信号能够在亚秒时间尺度上根据行为状态在鸣唱与运动之间切换。

文章按钮：
文章

文章链接：
https://journals.physiology.org/doi/10.1152/jn.00110.2021

论文：
鸣禽中的位置偏好与发声学习依赖不同的强化信号

作者：
Murdoch D, Chen R, Goldberg J.

期刊与年份：
Scientific Reports, 2018

简介：
表明同一噪声在位置偏好中可以是奖赏性的，但在发声学习中却是厌恶性的，揭示了彼此独立的强化系统。

文章按钮：
文章

文章链接：
https://www.nature.com/articles/s41598-018-25112-5

## 链接

分节肩题：
链接

分节标题：
链接。

邮箱标签：
邮箱

邮箱内容：
ruidong@mit.edu

邮箱说明：
直接联系

Google Scholar 标签：
Google Scholar

Google Scholar 内容：
全部论文

Google Scholar 说明：
论文列表

Google Scholar 链接：
https://scholar.google.com/citations?user=omTfD3gAAAAJ&hl=en&oi=ao

X 标签：
X

X 内容：
@ruidongchen

X 说明：
短动态

X 链接：
https://x.com/ruidongchen

Bluesky 标签：
Bluesky

Bluesky 内容：
@ruidong.bsky.social

Bluesky 说明：
公开动态

Bluesky 链接：
https://bsky.app/profile/ruidong.bsky.social

LinkedIn 标签：
LinkedIn

LinkedIn 内容：
陈睿东

LinkedIn 说明：
职业档案

LinkedIn 链接：
https://www.linkedin.com/in/ruidong-chen-b5174733/

知乎标签：
Zhihu

知乎内容：
Mon1st

知乎说明：
长文写作

知乎链接：
https://www.zhihu.com/people/mon1st

小宇宙标签：
小宇宙

小宇宙内容：
双币

小宇宙说明：
播客

小宇宙链接：
https://www.xiaoyuzhoufm.com/podcast/6817c030601614cb6bbd0d6a

ORCID 标签：
ORCID

ORCID 内容：
0000-0003-0163-979X

ORCID 说明：
学术身份

ORCID 链接：
https://orcid.org/0000-0003-0163-979X

McGovern 研究所标签：
McGovern 研究所

McGovern 研究所内容：
Jazayeri 实验室

McGovern 研究所说明：
实验室主页

McGovern 研究所链接：
https://mcgovern.mit.edu/tile/ruidong-chen/

## 页脚

页脚文案：
由 AI 协助制作，经本人校对。
